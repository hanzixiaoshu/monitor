...
def Oracle_Lock(sec_ip, value_port, keys):
    try:
        ...
        cursor = conn.cursor()
        sql = 'SELECT COUNT(*) FROM v$sqlarea a, v$session s,v$locked_object l, dba_objects db where s.prev_sql_addr = a.address and l.session_id = s.sid and db.object_id = l.object_id and s.SECONDS_IN_WAIT > 5 order by sid,s.serial#'
...
def RunAll():
    ...
    for sec_ip in sections_ip:
        ...
        for keys in iniconfig.options(sec_ip):
            ...
            #print(data_value)
            Push_Data(sec_ip, value_port, Connect_Port(sec_ip, int(value_port)),"Oracle_Alive")
